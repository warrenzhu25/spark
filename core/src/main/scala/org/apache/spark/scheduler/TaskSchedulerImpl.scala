...
private[spark] class TaskSchedulerImpl(
 ...
 private def getTaskAccumulableInfosAndProcessRate(
  ...
  private def logExecutorLoss(
    ...
    case ExecutorDecommission(_, reason) =>
      logInfo(s"Executor $executorId on $hostPort is decommissioned. $reason")
    case _: ExecutorDecommissionFinished =>
      logInfo(s"Executor $executorId on $hostPort decommission finished.")
    case _ =>
