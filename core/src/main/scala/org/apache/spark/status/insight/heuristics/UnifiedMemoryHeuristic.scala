 ...
 *
 * This heuristic reports the fraction of memory used/ memory allocated
 * and if the fraction can be reduced.
 * Also, it checks for the skew in peak unified memory and reports if the skew is too much.
 */
object UnifiedMemoryHeuristic extends Heuristic {
  ...
  class Evaluator(data: SparkApplicationData) {
    ...
    lazy val meanUnifiedMemory: Long = 0L
//      (executorList.map {
//      executorSummary => {
...
//    }.sum) / executorList.size
    ...
    lazy val maxUnifiedMemory: Long = 0L
//      executorList.map {
//      executorSummary => {
//        (executorSummary.peakUnifiedMemory.getOrElse(EXECUTION_MEMORY, 0).asInstanceOf[Number].longValue
//          + executorSummary.peakUnifiedMemory.getOrElse(STORAGE_MEMORY, 0).asInstanceOf[Number].longValue)
...
//    }.max
    ...
    // If sparkMemoryFraction or total Unified Memory allocated is less than their respective
    // thresholds then won't consider for severity
