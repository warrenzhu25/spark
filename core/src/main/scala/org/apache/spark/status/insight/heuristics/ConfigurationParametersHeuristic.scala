...
/**
 * A heuristic for recommending configuration parameter values,
 * based on metrics from the application run.
 */
...
object ConfigurationParametersHeuristic extends Heuristic {
   ...
   class Evaluator(data: SparkApplicationData) {
     ...
     lazy val sparkSqlShufflePartitions = appConfigurationProperties
       ...
       .map(_.toInt)
       .getOrElse(SPARK_SQL_SHUFFLE_PARTITIONS_DEFAULT)
     ...
     private lazy val driver: ExecutorSummary = executorSummaries
       .find(_.id == "driver")
       .orNull
     ...
     val severity = Severity.max(calculateStageSeverity(stageAnalysis),
     ...
     private def adjustExecutorMemory() = {
       if (((sparkExecutorMemory > SPARK_EXECUTOR_MEMORY_THRESHOLD_LOW &&
         ...
         if (sparkExecutorMemory <= SPARK_EXECUTOR_MEMORY_THRESHOLD_INCREASE_CORES &&
           ...
           val possibleCores = ((sparkExecutorMemory - SPARK_RESERVED_MEMORY) /
   ...
   private def calculateStageSeverity(analyses: Seq[StageAnalysis]): Severity = {
     val stageSeverities = analyses
       .map(_.getStageAnalysisResults.maxBy(_.severity.getValue).severity)
     if (stageSeverities.nonEmpty) {
   ...
   private def calculateRecommendedMemory(
     ...
     val calculatedMem = (jvmUsedMemory * Math.ceil(recommendedCores / numCores.toDouble) *
       (1 + DEFAULT_MEMORY_BUFFER_PERCENTAGE) + SPARK_RESERVED_MEMORY).toLong
   ...
   private def calculateTotalUnifiedMemory(
    ...
    /**
     * Given a memory value in bytes,
     * convert it to a string with the unit that round to a >0 integer part.
     *
