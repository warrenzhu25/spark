...
private[storage] class BlockManagerDecommissioner(
  ...
  // Number of shuffle blocks that failed to migrate due to errors
  private[storage] val failedShuffles = new AtomicInteger(0)

  // Map to track failure reasons and their counts
  private[storage] val failureReasons =
    new java.util.concurrent.ConcurrentHashMap[String, AtomicInteger]()

  /**
   * Record a failure reason and increment both the failed shuffle count and the specific
  ...
  private def buildShuffleStat(): MigrationStat = {
    // Convert ConcurrentHashMap to immutable Map for the case class
    val failureReasonsMap = failureReasons.asScala.map { case (reason, count) =>
      reason -> count.get()
    }.toMap
    MigrationStat(migratingShuffles.size - numMigratedShuffles.get(),
      ...
      failedShuffles.get(), failureReasonsMap)
  }
}
 ...
 * @param failedBlocks number of shuffle blocks that failed to migrate due to errors
 * @param failureReasons map of failure reason (exception type) to count of blocks that failed
 *                      with that reason
 */
