...
import org.apache.spark.deploy.master.WorkerState.{ALIVE, DECOMMISSIONED, DECOMMISSIONED_IDLE, IDLE, WorkerState}
...
private[spark] class WorkerInfo(
  ...
  def getState: WorkerState = state match {
    ...
    case DECOMMISSIONED if isIdle => DECOMMISSIONED_IDLE
    case s => s
  }
