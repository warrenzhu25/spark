...
private[deploy] class Master(
  ...
  private def recommissionWorkersOnHosts(hostnames: Seq[String]): Integer = {
    ...
    val workersToRemoveHostPorts = workersToRecomm.map(_.hostPort)
    logInfo(s"Recommissioning the decommissioned workers with" +
      s" host:ports ${workersToRemoveHostPorts}")

    // The workers are recommissioned async to avoid blocking the receive loop for the entire batch
    self.send(RecommissionWorkers(workersToRecomm.map(_.id).toSeq))

    // Return the count of workers actually recommissioned
    workersToRecomm.size
  }
