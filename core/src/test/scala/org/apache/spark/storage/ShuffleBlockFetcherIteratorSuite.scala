...
class ShuffleBlockFetcherIteratorSuite extends SparkFunSuite with PrivateMethodTester {
  ...
  test("SPARK-41956: Fetch Migrated blocks with all blocks failed when fetch failed" +
  ...
  test("SPARK-41956: Fetch Migrated blocks with blocks migrated twice when fetch failed" +
      " on a decommissioned executor") {
    // Make sure remote blocks would return
  ...
  test("SPARK-41956: Fetch Migrated blocks with some blocks missing when fetch failed" +
      " on a decommissioned executor") {
    // Make sure remote blocks would return
    val remoteBmId1 = BlockManagerId("test-client-1", "test-client-1", 2)
    val remoteBmId2 = BlockManagerId("test-client-2", "test-client-2", 2)
  ...
  test("SPARK-41956: Fetch Migrated blocks with some local blocks when fetch failed" +
      " on a decommissioned executor") {
    // Make sure remote blocks would return
    val blockManager = createMockBlockManager()
    val localBmId = blockManager.blockManagerId
