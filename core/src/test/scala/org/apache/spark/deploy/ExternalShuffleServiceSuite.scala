...
class ExternalShuffleServiceSuite extends SparkFunSuite {
  ...
  override def beforeAll(): Unit = {
    ...
    sparkConf.set("spark.shuffle.push.server.mergedShuffleFileManagerImpl",
  ...
  test("MergeFileManager should be created") {
    val mergeManager = externalShuffleService.getBlockHandler.getMergeManager
    assert(mergeManager != null)
    assert(mergeManager.isInstanceOf[RemoteBlockPushResolver])
  }
}
