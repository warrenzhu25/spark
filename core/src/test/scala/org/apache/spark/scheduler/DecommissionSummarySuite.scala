...
class DecommissionSummarySuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("ExecutorDecommissionFinished loss reason with DecommissionSummary") {
    ...
    // The message should use the detailed summary message with prefix
    val expectedMessage = ExecutorDecommissionFinished.msgPrefix + summary.toDetailedMessage
  ...
  test("ExecutorDecommissionFinished loss reason without DecommissionSummary") {
    ...
    assert(lossReason.message === ExecutorDecommissionFinished.msgPrefix + "Basic completion")
  }
