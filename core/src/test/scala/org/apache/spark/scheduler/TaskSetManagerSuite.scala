...
class TaskSetManagerSuite
  ...
  // Helper methods for shuffle skew filtering tests to reduce code duplication

  private def distributeTasksSimple(
      manager: TaskSetManager,
      totalTasks: Int,
      splitPoint: Int,
      exec1: String = "exec1",
      exec2: String = "exec2",
      host: String = "host1"): Unit = {
    val directTaskResult = createTaskResult(0)
    for (i <- 0 until totalTasks) {
      val execId = if (i < splitPoint) exec1 else exec2
      val task = manager.resourceOffer(execId, host, NO_PREF)._1.get
      manager.handleSuccessfulTask(task.taskId, directTaskResult)
    }
  }

  private def distributeTasksModulo(
      manager: TaskSetManager,
      totalTasks: Int,
      numExecutors: Int,
      execIdPrefix: String = "exec",
      hostPrefix: String = "host"): Unit = {
    val directTaskResult = createTaskResult(0)
    for (i <- 0 until totalTasks) {
      val execId = s"$execIdPrefix${i % numExecutors}"
      val host = s"$hostPrefix${i % numExecutors}"
      val task = manager.resourceOffer(execId, host, NO_PREF)._1.get
      manager.handleSuccessfulTask(task.taskId, directTaskResult)
    }
  }

  private def distributeTasksByMap(
      manager: TaskSetManager,
  ...
  private def createManagerWithConfigAndExecutors(
      taskCount: Int,
      numExecutors: Int,
      configRatio: Option[Double] = None): TaskSetManager = {
    configRatio.foreach { ratio =>
      sched.sc.conf.set(config.SHUFFLE_SKEW_MAX_EXECUTORS_RATIO, ratio)
    }
    val taskSet = FakeTask.createTaskSet(taskCount)
    val manager = new TaskSetManager(sched, taskSet, MAX_TASK_FAILURES)
    for (i <- 3 until numExecutors) {
      sched.addExecutor(s"exec$i", s"host$i")
    }
    manager
  }

  private def assertSkewedSize(
      manager: TaskSetManager,
      totalExecutors: Int,
      expectedSize: Int,
      message: String = ""): Unit = {
    val skewed = manager.getSkewedExecutors(totalExecutors)
    val fullMessage = if (message.isEmpty) {
      s"Expected size $expectedSize but got ${skewed.size}"
    } else {
      s"$message: expected size $expectedSize but got ${skewed.size}"
    }
    assert(skewed.size === expectedSize, fullMessage)
  }

  private def assertSkewedContains(
      manager: TaskSetManager,
      totalExecutors: Int,
      expectedExecIds: Set[String],
      message: String = ""): Unit = {
    val skewed = manager.getSkewedExecutors(totalExecutors)
    expectedExecIds.foreach { execId =>
      val fullMessage = if (message.isEmpty) {
        s"Expected $execId in $skewed"
      } else {
        s"$message: expected $execId in $skewed"
      }
      assert(skewed.contains(execId), fullMessage)
    }
  }

  private def assertSkewedEmpty(
      manager: TaskSetManager,
      totalExecutors: Int,
      message: String): Unit = {
    val skewed = manager.getSkewedExecutors(totalExecutors)
    assert(skewed.isEmpty, message)
  }

  test("getSkewedExecutors respects maxExecutorsNum absolute limit") {
    testExcludeShuffleSkewSetup(30, Some(true), Some(2), Some(3), Some(1.0))
    val manager2 = createManagerWithConfigAndExecutors(30, 10, Some(0.5))
    distributeTasksModulo(manager2, 30, 10)

    // With 10 executors and 50% ratio, would return 5, but absolute cap is 3
    assertSkewedSize(manager2, 10, 3, "Should return exactly 3 executors (absolute cap)")
  }

  test("getSkewedExecutors with maxExecutorsNum=0 returns empty") {
    val manager = testExcludeShuffleSkewSetup(10, Some(true), Some(2), Some(0), Some(1.0))
    distributeTasksSimple(manager, 10, 8, "exec1", "exec2")
    assertSkewedEmpty(manager, 3, "Zero cap should return empty set")
  }

  test("getSkewedExecutors handles invalid maxExecutorsNum gracefully") {
    val manager = testExcludeShuffleSkewSetup(10, Some(true), Some(2), Some(1), Some(1.5))
    distributeTasksSimple(manager, 10, 8, "exec1", "exec2")
    ...
    if (skewed.nonEmpty) {
      assertSkewedContains(manager, 2, Set("exec1"), "exec1 should be the skewed executor")
    }
  }

  test("getSkewedExecutors respects maxExecutorsRatio percentage limit") {
    testExcludeShuffleSkewSetup(30, Some(true), Some(2), Some(100), Some(1.0))
    val manager2 = createManagerWithConfigAndExecutors(30, 10, Some(0.2))
    distributeTasksModulo(manager2, 30, 10)

    // With 10 executors and 20% ratio, should return 2 (ratio wins over absolute 100)
    assertSkewedSize(manager2, 10, 2, "Should return 2 executors (20% of 10)")
  }

  test("getSkewedExecutors with maxExecutorsRatio=0.0 returns empty") {
    testExcludeShuffleSkewSetup(10, Some(true), Some(2), Some(10), Some(1.5))
    val manager2 = createManagerWithConfigAndExecutors(10, 3, Some(0.0))
    distributeTasksSimple(manager2, 10, 8, "exec1", "exec2")

    // Zero ratio should return empty
    assertSkewedEmpty(manager2, 3, "Zero ratio should return empty set")
  }

  test("getSkewedExecutors rounds up fractional ratio calculations") {
    testExcludeShuffleSkewSetup(33, Some(true), Some(2), Some(100), Some(1.0))
    val manager2 = createManagerWithConfigAndExecutors(33, 11, Some(0.10))
