...
class ExecutorMonitorSuite extends SparkFunSuite {
  ...
  test("dynamic shuffle timeout calculation") {
    ...
    // Write 100MB of shuffle data (100 * 1024 * 1024 bytes)
    val taskMetrics = TaskMetrics.empty
    ...
    // Dynamic timeout should be 100MB * 500ms/MB = 50,000ms = 50s
    val dynamicTimeout = 50 * 1000
    val dynamicDeadline = clock.getTimeMillis() + dynamicTimeout + 1

    // Should not timeout before dynamic deadline
    assert(monitor.timedOutExecutors(clock.getTimeMillis() + dynamicTimeout - 1000).isEmpty)
  ...
  test("dynamic timeout with minimum protection for small shuffle data") {
    conf
      .set(DYN_ALLOCATION_SHUFFLE_TRACKING_TIMEOUT.key, "240s")
      .set(DYN_ALLOCATION_SHUFFLE_TRACKING_ENABLED, true)
      .set(DYN_ALLOCATION_SHUFFLE_TRACKING_DYNAMIC_TIMEOUT_ENABLED, true)
    ...
    // Write only 1KB of shuffle data (much less than 1MB)
    val taskMetrics = TaskMetrics.empty
    taskMetrics.shuffleWriteMetrics.incBytesWritten(1024)
    monitor.onTaskEnd(SparkListenerTaskEnd(1, 1, "foo", Success, t1,
      new ExecutorMetrics, taskMetrics))
    monitor.onJobEnd(SparkListenerJobEnd(1, clock.getTimeMillis(), JobSucceeded))

    // Should use idle timeout (60s) as minimum, not tiny dynamic timeout
    assert(monitor.timedOutExecutors(idleDeadline) === Seq("1"))
    val smallDynamicDeadline = clock.getTimeMillis() + 1 // 1ms would be too small
    assert(monitor.timedOutExecutors(smallDynamicDeadline).isEmpty)
  }
  ...
  test("timeout calculation with both cached RDD and shuffle data") {
    ...
    assert(monitor.timedOutExecutors(storageDeadline) === Seq("1"))
    val dynamicDeadline = clock.getTimeMillis() + 100 * 1000 + 1
  ...
  test("invalid configuration values are rejected") {
    ...
    intercept[IllegalArgumentException] {
      new ExecutorMonitor(negativeConf, client, null, clock, allocationManagerSource())
    }
    ...
    intercept[IllegalArgumentException] {
      new ExecutorMonitor(zeroConf, client, null, clock, allocationManagerSource())
    }
  }
