...
private[continuous] class EpochCoordinator(
  ...
  val INSTRUCTION_FOR_FEWER_CORES =
    """
      |Total %s kafka partitions to read, but only have %s(num of executors) * %s(cores per executor) = %s(total cores).
  ...
  private def checkTotalCores(): Unit = {
    ...
    logDebug(s"Check total cores $totalCores and partition number $numReaderPartitions")
