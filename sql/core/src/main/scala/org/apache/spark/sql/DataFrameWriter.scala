...
class DataFrameWriter[T] private[sql](ds: Dataset[T]) {
  ...
  private[sql] val df: DataFrame = ds.toDF()
  ...
  private[sql] var source: String = df.sparkSession.sessionState.conf.defaultDataSourceName

  private[sql] var mode: SaveMode = SaveMode.ErrorIfExists
